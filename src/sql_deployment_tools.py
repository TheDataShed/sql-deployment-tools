...
class sql_deployment:
    ...
    def deploy(
        ...
        If not supplied we'll attempt to use environment variable `CONNETION_STRING` \n
        ...
        if not connection_string:
            raise ValueError(
                """Missing connection string. Can either be supplied using the `--connection-string`
                argument or via an environment variable `CONNETION_STRING`"""
            )
